...
import {getPagingAxqStaffLoginLog,deleteAxqStaffLoginLog,batchDeleteAxqStaffLoginLog } from './api';
import AxqStaffLoginLogModal from './modal.vue';
import {Message,MessageBox,router,parseResult,formatColTime} from "@/adapter";

// AxqStaffLoginLog数据映射类
class ListModel {
    constructor() {
      this.id = 0;// Id
      ...
      this.create_time = <unknown>;// CreateTime
      this.update_time = <unknown>;// UpdateTime
      this.delete_time = <unknown>;// DeleteTime
...
const queryPagingData = async (args)=> {
    const { data } = await getPagingAxqStaffLoginLog(list.page,list.rows,queryParams)
...
// 新增数据
const handleCreate = ()=> openForm(null,"新增AxqStaffLoginLog")

// 编辑数据
const handleEdit = (row)=>openForm(row,"编辑AxqStaffLoginLog")

// 打开表单
const openForm = (row,title)=>{
  const id = row?row.id:0
  
  // router.push({path:"../axq/staff_login_log/detail",query:{title,id}});

  // 使用模态框以编辑数据
  dialog.open = true
  dialog.title = title
  dialog.params = id
  dialog.modal = AxqStaffLoginLogModal
}
...
const handleDelete = ({$index,row}) => {
    MessageBox.confirm('执行此操作数据无法恢复,是否继续?', '提示', {
        ...
        let ret = await (row && row.id?
          deleteAxqStaffLoginLog(row.id):
          batchDeleteAxqStaffLoginLog(selectedIds.value))
