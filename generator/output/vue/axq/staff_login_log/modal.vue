<template>
  <div class="mod-form-container">
    <el-form ref="formRef" class="mod-form" size="small"
        ...
        <el-col :md="12" :xs="24">
            <el-form-item class="mod-form-item" label-position="left" label-width="85px" label="Ip:" prop="ip">
            <el-input type="textarea" v-model="formData.ip" class="mod-form-input" :autosize="{ minRows: 2, maxRows: 4}" placeholder=""/>
                <span class="mod-form-remark"></span>
            </el-form-item>
          </el-col>
        <el-col :md="12" :xs="24">
...
import {AxqStaffLoginLog,getAxqStaffLoginLog,createAxqStaffLoginLog,updateAxqStaffLoginLog } from "./api"
...
let formData  = ref(new AxqStaffLoginLog()); 
...
const fetchFormData = async(id) =>{
  try {
    const { data } = await getAxqStaffLoginLog(id, { /* Your params here */ });
    formData.value = data;
  } catch (err) {
    console.error(err);
...
const submitForm = ()=> {
  formRef.value.validate(async valid => {
    ...
    let ret = await (props.value?updateAxqStaffLoginLog(props.value,formData.value):createAxqStaffLoginLog(formData.value))
